...
subroutine AQZfc_dipole_wrapper(q, mu, natoms, ndims) bind ( C, name="AQZfc_dipole_wrapper")
   use, intrinsic:: iso_c_binding

   integer(c_int), intent(in) :: natoms, ndims 
   real(c_double), dimension(natoms, ndims), intent(in) :: q 
   real(c_double), dimension(natoms, ndims) :: temp
   real(c_double), dimension(ndims), intent(out) :: mu

    ! convert atomic units (Bohr) to angstrom
    temp = q * 0.529177210903
    call DIPOLE_XY3(temp, mu)

end subroutine AQZfc_dipole_wrapper
...
! Parameters:
   ...
   double precision  :: paramtmp, charge, EC, Xshift, VCPRM !,mu(3)
   !DMS_A, dipol_xy,
   ...
   double precision, dimension(4, 3) :: x ! the 4th dim is not accessed, could be an error
